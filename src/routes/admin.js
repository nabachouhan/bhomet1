...
router.post("/publish-tiff", upload.none(), adminAuthMiddleware, async (req, res) => {
  ...
  try {
    ...
    // 1️⃣ Validate file exists BEFORE calling GeoServer
    ...
    // 2️⃣ Create GeoTIFF coverage store
    ...
    // 3️⃣ Axios / GeoServer error handling
    if (error.response) {
...
router.post("/delete", adminAuthMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (file_type === "raster") {
      ...
      // 3️⃣ Delete GeoTIFF from disk (MATCH upload path)
      const rasterPath = path.join(
